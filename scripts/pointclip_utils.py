# from https://github.com/ZrrSkywalker/PointCLIP/blob/main/trainers/mv_utils_fs.py
...
RESOLUTION = 196  # changed from 128, but 196 is used for scanobjectnn
TRANS = -1.8  # changed from 1.6, but 1.8 is used for scanobjectnn
...
def euler2mat(angle):
    ...
    zero = z.detach() * 0
    one = zero.detach() + 1
    zmat = torch.stack([cosz, -sinz, zero, sinz, cosz, zero, zero, zero, one], dim=_dim).reshape(
        _view
    )
    ...
    ymat = torch.stack([cosy, zero, siny, zero, one, zero, -siny, zero, cosy], dim=_dim).reshape(
        _view
    )
    ...
    xmat = torch.stack([one, zero, zero, zero, cosx, -sinx, zero, sinx, cosx], dim=_dim).reshape(
        _view
    )
...
def distribute(depth, _x, _y, size_x, size_y, image_height, image_width):
    ...
    (image_height, image_width) in a region
    ...
    # [batch, num_points, size_x, size_y]
    torch.abs(
        (extended_x - _x.unsqueeze(2).unsqueeze(3)) * (extended_y - _y.unsqueeze(2).unsqueeze(3))
    )
...
# adapted from https://github.com/ZrrSkywalker/PointCLIP/blob/main/trainers/zeroshot.py
...
class TextualEncoder(nn.Module):
    def __init__(self, classnames, clip_model, num_views=6):
        super().__init__()
...
class PointCLIP_ZS:
    def build_model(self, clip_model, classnames, num_views):

        # Encoders from CLIP
        self.visual_encoder = clip_model.visual
        self.textual_encoder = TextualEncoder(classnames, clip_model, num_views)

        self.logit_scale = clip_model.logit_scale
        self.dtype = clip_model.dtype
        self.channel = clip_model.visual.output_dim
    ...
    def mv_proj(self, pc):
        ...
        img = torch.nn.functional.upsample(
